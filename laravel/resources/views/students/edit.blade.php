...
@section('content')
    <div class="w-[90vw] md:w-[70vw] bg-white drop-shadow-lg rounded-xl p-4 bg-gradient-to-b from-slate-100 to-white ">
        ...
        <h1 class="text-center font-bold text-2xl py-2">Edit a Student</h1>
        <form action='{{ route('students.update',$student->id) }}' method="post"
            ...
            <div class="w-1/3">
                ...
                <input type="text" placeholder="Enter course name" id="course" name="course"
                   value="{{old('course',$student->course)}}" class="w-full py-2 px-3 rounded-lg border border-blue-200">
                @error('course')
                    <p class="text-red-400 text-sm">{{ $message }}</p>
                @enderror
            </div>
            <div class="w-1/3">
                ...
                value="{{old('age',$student->age)}}" class="w-full py-2 px-3 rounded-lg border border-blue-200">
                @error('age')
                    <p class="text-red-400 text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="w-1/3">
                <button
                    ...
                    <span class="text-2xl font-bold">+</span>Update
                </button>
            </div>
        </form>
    </div>
